...
android:authorities="com.you.company.SomeProvider"/>{% endhighlight %}

授权之后ContentProvider就拥有了以授权前缀开头的URL：

{% highlight java %}
content://com.your.company.SomeProvider/{% endhighlight %}

ContentProvider还提供了一种类似于REST的URL来获取或操作数据。对于前面的注册，标识SomeProvider的数据库中某一项的集合的URL为：

{% highlight java %}
content://com.your.company.SomeProvider/collection{% endhighlight %}

如果在SomeProvider的Collection下还有具体的Item，那么的他的URL为：

{% highlight java %}
content://com.you.company.SomeProvider/collection/#{% endhighlight %}
...
{% highlight java %}vnd.android.cursor.item/vnd.yourcompanyname.contenttype{% endhighlight %}

而对于多条记录或者是行的集合，MIME的类型是类似于这样：

{% highlight java %}vnd.android.cursor.dir/vnd.youcompany.contenttype{% endhighlight %}
